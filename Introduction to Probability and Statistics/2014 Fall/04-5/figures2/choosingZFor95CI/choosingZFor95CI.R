...
pdf('choosingZFor95CI.pdf', 1.2*6.3, 1.2*3.2)
...
#      labels=c(as.character(-zalpha), as.character(zalpha)  ),
#      cex=5,
...
text(0, yMax, "0.95", pos=3, cex=1.1)

lines(rep(-zalpha,2), c(0,1.25*yMax), lty=2, col='#00000055', lwd=1.5)
lines(rep( zalpha,2), c(0,1.25*yMax), lty=2, col='#00000055', lwd=1.5)

text(-zalpha, yMax/3.5, as.character(-zalpha), cex=1.1)
text(zalpha, yMax/3.5, paste("+", as.character(+zalpha), sep=""), cex=1.1)

arrows(zalpha*-1, yMax, -3.5, yMax, lwd=2, code = 3, length=0.15)
#text( (zalpha*-1-3.5)/2, yMax, expression( frac(alpha, 2) ), pos=3, cex=1.1)
text( (zalpha*-1-3.5)/2, yMax, "0.025", pos=3, cex=1.1)
