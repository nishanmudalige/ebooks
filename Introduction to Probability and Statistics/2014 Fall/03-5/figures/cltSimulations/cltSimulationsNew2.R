...
pdf('cltSimulationsNewest2.pdf', 6.8, 8)

#layout(matrix(1:20, 5, byrow=TRUE), widths=c(0.85,2,2,2), heights=c(2,2,2,2,2))
layout(matrix(1:24, 6, byrow=TRUE), widths=c(0.85,2,2,2), heights=c(2,2,2,2,2,2))
...
par(mar=c(3, 1, 0, 1), mgp=c(3, 1, 0), las=0)
...
at[[5]] <- seq(0.75, 1.25, 0.25)
for(i in 1:length(n)){
  plot(0:1, 0:1, axes=FALSE, type='n')
  text(0.5, 0.3, paste('n =', n[i]), cex=1.5)
