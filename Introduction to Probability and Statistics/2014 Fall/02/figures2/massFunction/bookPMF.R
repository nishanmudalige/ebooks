...
probDist <- function(x, prob, labels1=NA,
	...
	#axis(2)
	make.bar(x, prob, thickness=0.1, col=col)
	#abline(h=0)
}
...
#		y <- c(0, height[i], height[i], 0, 0)
		segments(x1[i], 0, x2[i], height[i], col=col, lty=2)
		points(x1[i], height[i], col=col, pch=19, lwd=5)
	}
}

pdf('bookCostMassFunction.pdf', 5, 3)
at <- c(0, 137, 170)
prob <- c(0.2, .55, .25)
