...
myPDF('vennUnion.pdf', 2.95, 1.75, mar=c(0.2, 0.2, 0.6, 0.2))
plot(c(0.2,1.8), c(0,1), type='n', axes=FALSE, xlab="", ylab="")
box()

z <- seq(0,2*pi, len=1000)
x1 <- cos(z)/2.2+0.7
y1 <- sin(z)/2.2+0.5
...
cord.x1 <- x1[which(x1<1)]
cord.y1 <- y1[which(x1<1)]
cord.x2 <- x2[which(x2>1)]
cord.y2 <- y2[which(x2>1)]
