...
Probability forms a foundation for statistics. You might already be familiar with many aspects of probability, however, formalization of the concepts is new for most. 
In a probability problem certain characteristics of the population, such as the values of parameters are either known or at least assumed to be known.
We then use this information to answer questions regarding observing 
specific outcome associated with a sample drawn from that population.
This chapter aims to introduce probability on familiar terms using processes most people have seen before.
...
We start by defining an ``experiment'' in the context of probability. 
This term with regards to probability is not quite the same as with every day English.


\begin{termBox}{\tBoxTitle{Experiment}
...
%There are several more definitions that we should define in order for our understanding to be rigorous.

\begin{termBox}{\tBoxTitle{Sample Space}
A sample space is the set of all possible outcomes of an experiment.
}
\end{termBox}


\begin{termBox}{\tBoxTitle{Sample Point}
A sample point is an element of of a sample space.
}
\end{termBox}

A sample space describes any and all outcomes that can occur.
A sample space can be a finite set or an infinite set.
The symbol $\Omega$ is often used to denote a sample space.
The experiment of rolling a die produces a value in the set $\{$\resp{1}, \resp{2}, \resp{3}, \resp{4}, \resp{5}, \resp{6}$\}$. 
%This set of all possible outcomes is the \term{sample space} ($\Omega$)\marginpar[\raggedright\vspace{-5mm}
This set of all possible outcomes is the sample space ($\Omega$)\marginpar[\raggedright\vspace{-5mm}

$\Omega$\\\footnotesize Sample space]{\raggedright\vspace{-5mm}

$S$\\\footnotesize Sample space}\index{S@$\Omega$} for rolling a die. 
The outcomes $\resp{1}, \resp{2}, \resp{3}, \resp{4}, \resp{5}, \resp{6}$ are sample points.

%We often use the sample space to examine the scenario where an event does not occur.



\begin{termBox}{\tBoxTitle{Event}
An event is any collection of sample points.
}
\end{termBox}

In the example of rolling a die, we can consider an event to be obtaining a $\resp{3}$ 
on a roll as an event or obtaining an even number on a roll as an event. 
With this framework in place, we can attach probabilities to events occurring within a sample space.
We now give the formal definition of probability. %\term{probability}.

\begin{termBox}{\tBoxTitle{Probability}
The \term{probability} of an event is the proportion of times the event would occur if we observed the random process an infinite number of times.}
\end{termBox}
...
\begin{example}{Consider $p$ as the probability of rolling a \resp{1} (as above). 
We can also write this probability as}
\begin{eqnarray*}
P(\text{rolling a \resp{1}})
\end{eqnarray*}
\label{probexampledice}
\end{example}
...
We always write statements regarding
probabilities with capital $P$ along with something meaningful within the context of a probability problem
in parentheses 
such as in in Example $\ref{probexampledice}$.
The information in parentheses could be an event or a sample point or a combination of events etc.
We can write long statements in words but it is more elegant to use certain symbols.
A list of common symbols is given in Table~\ref{CommonNotation}.
%By this we mean that a probability statement is of the form
%$P(\cdot)$ where  $\cdot$ is something meaningful within the context of a probability problem.
...
\begin{termBox}{\tBoxTitle{Common symbols in set theory} 

\begin{table}[H]
\centering
%\begin{tabular}{c C{0.2cm} L{9cm}}
\begin{tabular}{c c l}
$\cap$		&	:	&	Intersection	\\
$\cup$		&	:	&	Union		\\
...
\end{tabular}
\caption{Common notation used in set theory.}
\label{CommonNotation}
\end{table}
...
The symbols in Table~\ref{CommonNotation} become very useful when
we work with events and write statements for probability problems.

Their interaction refers to elements that are in both $A$ and $B$.
Their union refers  to elements that are in $A$ or $B$.
The complement of set $A$ refers to all elements except for those in set $A$.
We talk more about complements in Section~\ref{probabilityComplement}.
If $A \subset B$, this means that $B$ contains all the elements of $A$ as well as
some additional elements.
If $A \subseteq B$, this means that $A$ is equal to $B$ or $B$ contains all the elements of $A$ as well as
some additional elements.
If we write $x \in A$ this means that $x$ is an element of $A$ and 
if we write $x \not\in A$ means that $x$ is not an element of $A$.


\begin{termBox}{\tBoxTitle{Common symbols in set theory with events and sample points} 
Let $A$ and $B$ be any 2 events in sample space $\Omega$.
\begin{table}[H]
\centering
%\begin{tabular}{c C{0.2cm} L{9cm}}
...
$A^{\text{c}}$	&	:	&	Everything except $A$\\
$A \subset B$	&	:	&	$A$ is completely contained in $B$\\
$A \subseteq B$&	:	&	$A$ is contained in $B$	\\
$x \in A$		&	:	&	$x$ is an element of $A$\\
$x\not\in A$	&	:	&	$x$ is not an element of $A$\\
\end{tabular}
\caption{Common notation used in set theory when we have two events.}
\label{CommonNotationWithAandB}
\end{table}
...
\term{Venn diagrams} are a pictorial way to represent a sample space, events and sample points.
They are useful when outcomes can be categorized as ``in'' or ``out'' for two or three variables, attributes, or random processes. 
Venn diagrams are an effective way of easily illustrating the concepts of set notation 
in Table~\ref{CommonNotation} or $\ref{CommonNotationWithAandB}$ 
such as intersections, unions and complements.

Let $A$ and $B$ be any 2 events in sample space $\Omega$.
A few examples of representing  $A$ and $B$ along with 
some of the symbols in Table~\ref{CommonNotation}
are given in Figures~\ref{vennIntersection},
~\ref{vennUnion} and ~\ref{vennComlement} below.

\begin{figure}[ht]
\centering
\begin{minipage}[b]{0.45\linewidth}
\centering
\includegraphics[height=1.50in]{02/figures2/venn/vennIntersection.pdf}
\caption{Shaded area represents $A \cap B$}
\label{vennIntersection}
\end{minipage}
\quad\quad\quad
\begin{minipage}[b]{0.45\linewidth}
\centering
\includegraphics[height=1.50in]{02/figures2/venn/vennUnion.pdf}
\caption{Shaded area represents $A \cup B$}
\label{vennUnion}
\end{minipage}
\end{figure}
...
%\begin{minipage}[b]{0.45\linewidth}
%\centering
%\includegraphics[height=1.50in]{02/figures2/venn/vennComplement.pdf}
%\caption{Shaded area represents $A^{\text{c}}$ }
%\label{vennIntersection}
%\end{minipage}
%\quad\quad\quad
%\begin{minipage}[b]{0.45\linewidth}
...
We can use all sorts of combinations of the symbols in Table~\ref{CommonNotation} to represent 
useful statements regarding probabilities. 
...
\begin{termBox}{\tBoxTitle{Addition Rule of disjoint outcomes} If $A$ and $B$ represent two disjoint outcomes, then the probability that one of them occurs is given by
%\begin{eqnarray*}
...
The Venn diagram in Figure~\ref{venn} uses a circle to represent diamonds and another to represent face cards. If a card is both a diamond and a face card, it falls into the intersection of the circles. If it is a diamond but not a face card, it will be in part of the left circle that is not in the right circle (and so on). The total number of cards that are diamonds is given by the total number of cards in the diamonds circle: $10+3=13$. The {\color{oiB}probabilities} are also shown (e.g. $10/52 = 0.1923$).

\begin{figure}
\centering
\includegraphics[height=1.4in]{02/figures/venn/venn}
\caption{Venn diagram for diamonds and face cards.}
\label{venn}
\end{figure}
...
\label{probabilityComplement}


We often use the sample space to examine the scenario where an event does not occur.
Recall that the complement of an event is the case in which the event does not occur.
For example, let $D=\{$\resp{2}, \resp{3}$\}$ represent the event that the outcome of a die roll is \resp{2} or \resp{3}. Then the \term{complement}\marginpar[\raggedright\vspace{0.2mm}

$A^c$\\\footnotesize Complement\\of outcome $A$]{\raggedright\vspace{0.2mm}

$A^c$\\\footnotesize Complement\\of outcome $A$}\index{Ac@$A^c$} of $D$ represents all outcomes in our sample space that are not in $D$, which is denoted by $D^c = \{$\resp{1}, \resp{4}, \resp{5}, \resp{6}$\}$. That is, $D^c$ is the set of all possible outcomes not already included in $D$. Figure~\ref{complementOfD} shows the relationship between $D$, $D^c$, and the sample space $\Omega$. 

\begin{figure}[hht]
\centering
\includegraphics[width=0.5\textwidth]{02/figures/complementOfD/complementOfD}
\caption{Event $D=\{$\resp{2}, \resp{3}$\}$ and its complement, $D^c = \{$\resp{1}, \resp{4}, \resp{5}, \resp{6}$\}$. $\Omega$~represents the sample space, which is the set of all possible events.}
\label{complementOfD}
\end{figure}
...
%\indent
%(i) every possible outcome not in $A$ is in $A^c$\\
%\indent
%(ii) $A$ and $A^c$ are disjoint\\

\begin{enumerate}
\item Every possible outcome not in $A$ is in $A^c$ \label{complementProperty1}
\item $A$ and $A^c$ are disjoint \label{complementProperty2}
\end{enumerate}

\noindent
%Property (i) implies
Property~\ref{complementProperty1} implies
\begin{eqnarray}
P(A\text{ or }A^c) = 1
...
\begin{eqnarray}\label{complement}
	P(A) + P(A^c)  & = & 1
\end{eqnarray}
An equivalent but more useful way to write expression $\ref{complement}$ is
\begin{eqnarray}
	P(A^c)  & = & 1- P(A)
...
A \term{probability distribution} is a table (i.e. a function) of all disjoint outcomes and their associated probabilities. 
...
Table~\ref{diceProb} shows the probability distribution for the sum of two dice. 
...
There are some special probability distributions that we will learn about in later sections of this text 
in Section~\ref{randomVariablesSection} on random variables.
...
If we have more information regarding a scenario then the probability of an event may change.
For example the probability of a student taking a course might increase if they find out 
that their instructor has a reputation for giving easy tests.
As such we have the notion of conditional probability.


\begin{termBox}{\tBoxTitle{Conditional Probability}
The conditional probability of the outcome of interest $A$ given condition $B$ 
has occurred is computed as: \vspace{-1.5mm}
\begin{eqnarray}
...
The expression $P(A | B)$ is the probability of event $A$ occurring given 
(i.e on the condition) that event $B$ has occurred.
The vertical bar ``$|$'' is read as \emph{given}.
We are essentially examining a restricting the sample space in which our focus shifts from all of
\marginpar[\raggedright%\vspace{-10mm}
$P(A | B)$\vspace{1mm}\\\footnotesize Probability of\\outcome $A$\\given $B$]{\raggedright\vspace{-10mm}
$P(A | B)$\vspace{1mm}\\\footnotesize Probability of\\outcome $A$\\given $B$}
 $\Omega$ to just $B$.
The only way that $A$ can occur in our restricted sample space of $B$ is
if we examine $P(A \cap B)$.
We will discuss this concept some more with an example.
...
\begin{termBox}{\tBoxTitle{Statistical independence}
Let $A$ and $B$ be any 2 events in sample space $\Omega$.
Events $A$ and $B$ are independent if and only if
\begin{eqnarray}
P(A | B) & = & P(A)	\label{indepA}	\\
P(B | A) & = & P(B)	\label{indepB}	\\
P(A \cap B) & = & P(A) \cdot P(B)	\label{indepAB}
\end{eqnarray}
\vspace{-6mm}
}
\end{termBox}

If 2 events are independent then $\ref{indepA}$, $\ref{indepB}$ and $\ref{indepAB}$ must all occur.
...
P(Y=\text{\resp{1}}|X=\text{\resp{1}})
	...
	&=& \frac{P(Y=\text{\resp{1}}) \cdot \color{oiGB}P(X=\text{\resp{1}})}{\color{oiGB}P(X=\text{\resp{1}})} \\
	&=& P(Y=\text{\resp{1}}) \\
\end{eqnarray*}
...
Bayes' Theorem is given more explicitly in the text box below.

\begin{termBox}{\tBoxTitle{Bayes' Theorem: inverting probabilities}
Let $A_{1}, A_{2}, \ldots, A_{n}$ be mutually exclusive events in sample space $\Omega$ such that 
$\displaystyle \sum_{i = 1}^{n} P(A_{i}) = 1$. Let $B$ be any event in $\Omega$. Then:
\begin{eqnarray}\label{BayesTheoremExplicitGeneral}
	P(A_{i} | B)	& = &	\frac{ P(A_{i}) \cdot P( B | A_{i}) }{ \displaystyle\sum_{j=1}^{n} P(A_{j}) \cdot P(B | A_{j}) }
\end{eqnarray}
}\index{Bayes' Theorem|textbf}
\end{termBox}

Bayes' Theorem is useful formula when manipulating conditional probabilities.
Equation \ref{equationOfBayesTheorem} means is that if we are provided information about 
$P(B|A_{i})$, $P(B)$ and $P(A)$, we can recover $P(B_{i}|A)$.

Although~\ref{BayesTheoremExplicitGeneral} may look overwhelming it is quite intuitive 
once you become more familiar with its use.
...
and
\begin{eqnarray}
	P(A | B) & = & \frac{ P(B|A) \cdot P(A) }{ P(B|A) \cdot P(A)  + P(B|A^{\text{c} } ) \cdot P(A^{\text{c} }) }
\end{eqnarray}
...
\begin{example}{}\label{exerciseForParkingLotOnCampusBeingFullAndWhetherOrNotThereIsASportingEvent}
...
\begin{example}{Here we solve the same problem presented in Example~\ref{exerciseForParkingLotOnCampusBeingFullAndWhetherOrNotThereIsASportingEvent}, except this time we use Bayes' Theorem.}
...
A \term{random variable} is the realization of what we previously called an experiment
in which we obtain a numerical outcome.
The term might be a little misleading since a random variable is actually a function.
...
A random variable $X$ is a function that maps events in $\Omega$ to a real number.
\begin{align}
...
The definition above is not a completely correct definition but it is sufficient for the purposes of an introductory
course in statistics.
We can consider a random variable to be a well defined map from our sample space to the real number line
where each event in the sample space is mapped on to a some number.
The proper definition involves a mathematical set of events known as a sigma algebra (or a sigma field).
The definition involving sigma algebra is something that is more suited for a course in
mathematical statistics or mathematical analysis (such as measure theory).\footnote{For a thorough understanding, we should also be familiar with another mathematical object called an ``algebra''
because a sigma algebra is not an algebra.}


We usually denote a random variable with a capital letter (such as ``$X$'') and the values it takes with a simple letter (such as $x$).
The values that the random variable $X$ can take is called the \term{support} of the random variable.
Regarding notation, there are several well known probability distributions that a random variable can follow.
Several of these distributions are discussed in Sections~\ref{sectionDistributionsOfRandomVariables}.
If a random variable is follows a known (or well defined) distribution we usually denote this using
the tilde ($\sim$) symbol.

\begin{termBox}{\tBoxTitle{Notation on distribution of a random variable}
Let $X$ be a random variable which follows a probability distribution $p(x)$.
We say that $X$ is distributed with distribution $p(x)$ and write this as
\begin{align}
	X \sim p(x)
\end{align}
}
...
There are 2 types of random variables which are
discrete random variable and continuous random variable.
Both of these are discussed in more detail in Sections~\ref{DiscreteRandomVariablesSection} 
and~\ref{ContinuousRandomVariablesSection}.



\subsection{Introduction of the mean and variance of random variables}
\label{sectionMeanAndVarianceOfRVs}

At this point we would like to introduce the definition of mean
and variance for random variables.
These definitions will be repeated and used later but we introduce them in order to get familiar with
the notation that we use as well to provide a gentle introduction to any subsequent courses in mathematical statistics. 
The box below is also a nice summary of the mean and variance of discrete and continuous random variables.
The mean of a random variable is also called the \term{expected value}.


\begin{termBox}{\tBoxTitle{Mean and variance of a random Variable}
...
E(X) = \mu =  \left\{ 
	\begin{array}{l l r}
    	 \displaystyle\sum_{i=1}^{n}  x_i \cdot P(X=x_i)	
	 		& \quad\quad \text{if $X$ is discrete} 	\\[2mm]	%	& (\theequation)\\[2mm]
	 \hfill\\
	 \displaystyle\int_{- \infty}^{+ \infty} x \cdot f(x) \, dx
	 		& \quad\quad \text{if $X$ is continuous}	%	& (\theequation)
	 \end{array}
\right.	%\label{meanBinomial}
\end{equation}

\hfill\\
\hfill\\
%The variance of $X$ is\\
The variance of $X$, labeled $\sigma^{2}$\index{Greek!sigma^{2} ($\sigma^{2}$)}, is\\
%\begin{align*}
%a  & = \left\{
%    \begin{array}{ll}
%      i = x^2 + y^2 \\ 
...
%\end{align*}
\begin{align}
V(X) = \sigma^{2}	& = E \big[ \big(X - E(X) \big)^2 \big]\label{varianceOfRV}		\\[2mm]
				& = \left\{
					\begin{array}{l l r}
						\displaystyle\sum_{i=1}^{k} (x_i - \mu)^2 \cdot P(X=x_i)
							& \quad\quad \text{if $X$ is discrete} \\% & (\theequation)\\[2mm]
						\hfill\\
						\displaystyle\int^{+ \infty}_{- \infty} (x - \mu)^{2} \cdot f(x) \, dx	
							& \quad\quad \text{if $X$ is continuous} \\%& (\theequation)\\
					\end{array}
					\right. %\nonumber
\end{align}
%\[ V(X) = \sigma^{2}  =  E[ \big(X - E(X) \big)^2 ]	\\
%\begin{align}
% V(X) = \sigma^{2}  & =  E[ \big(X - E(X) \big)^2 ]	\\
%& \[				  =
%	\left\{ 
%	\begin{array}{l l}
%    	\displaystyle\sum_{i=1}^{k} (x_i - \mu)^2 \cdot P(X=x_i)		& \quad\quad \text{if $X$ is discrete}\\[2mm]
%	\hfill\\
%	\displaystyle\int^{+ \infty}_{- \infty} (x - \mu)^{2} \cdot f(x) \, dx	& \quad\quad \text{if $X$ is continuous}\\
%	\end{array} \right.\]	%\label{meanBinomial}
%
%\end{align}
%\[ E(X) = \mu =  \left\{ 
%	\begin{array}{l l r}
...
%\begin{align}
%V(X) = \sigma^{2} 	& = E(X^2) - [E(X)]^{2} \\[2mm]
%				& = \bigg( \int_{-\infty}^{+\infty} x^{2} \cdot f(x) \bigg)- \mu^{2} \label{varianceofContinuousRVeasier}
%\end{align}



The mean $\mu$ of the random variable represents the average outcome.
In physics, the expectation holds the same meaning as the center of gravity. The distribution can be represented by a series of weights at each outcome, and the mean represents the balancing point. 
In terms of a physical interpretation, the variance of a random variable can be considered as the moment of inertia around a vertical axis around the mean (which is the center of mass of the distribution).

Note that with some simple manipulation an easier way to compute the variance of a simple random variable
is by using
%\begin{align}
%%V(X) = \sigma^{2} 	& = \mathbb{E}[ \big(X - \mathbb{E}(X) \big)^2 ] \\
%%				& = \int^{+ \infty}_{- \infty} (x - \mu)^{2} \cdot f(x) \, dx	\label{varianceofContinuousRV}
%V(X) = \sigma^{2} 	& = E[ \big(X - E(X) \big)^2 ] \\[2mm]
%				& = \int^{+ \infty}_{- \infty} (x - \mu)^{2} \cdot f(x) \, dx	\label{varianceofContinuousRV}
%\end{align}
\begin{align}
%V(X) = \sigma^{2} 	& = E(X)^{2} - [E(X)]^{2}	\label{varianceofContinuousRVeasier}
V(X) = \sigma^{2} 	& = E(X)^{2} - \mu^{2}	\label{varianceOfRVeasier}
\end{align}
It is possible to obtain~\ref{varianceOfRVeasier} from~\ref{varianceOfRV}
with some simple manipulation using properties of expectation.

\begin{tipBox}{\tipBoxTitle{Formulating the variance of a random variable}
In many cases it is usually easier to calculate the variance of a random variable
using~\ref{varianceOfRVeasier} rather than~\ref{varianceOfRV}.
This is true for both discrete as we as continuous random variables.}
\end{tipBox}


%\begin{exercise} 
%Verify that~\ref{varianceofContinuousRV} and~\ref{varianceofContinuousRVeasier} are equivalent forms of calculating the variance of a continuous random variable.
%\end{exercise}

\begin{exercise}{Verify that~\ref{varianceOfRV} and~\ref{varianceOfRVeasier} are  equivalent forms of calculating the variance of a continuous random variable 
[Note: This may be a hard question for students in an introductory course].}
\end{exercise}
%\begin{align}
%E(X) 	= \mu & = \sum_{i=1}^{n}  x_i \cdot P(X=x_i) 	\label{expectedValueofDiscreteRV} \\
%		  & = x_1 \cdot P(X=x_1) + x_2 \cdot P(X=x_2) +\ldots + x_n \cdot P(X=x_n) %\notag
%\end{align}
%
%
%
...
%V(X) = \sigma^2	&= \sum_{i=1}^{k} (x_i - \mu)^2 \cdot P(X=x_i)	\label{varianceofDiscreteRV}	\\
%				& = (x_1-\mu)^2 \cdot P(X=x_1) %+ (x_2-\mu)^2 \cdot P(X=x_2) 
%					+ \ldots + (x_n-\mu)^2 \cdot P(X=x_n)
%\end{align}
%The standard deviation of $X$, labeled $\sigma$\index{Greek!sigma ($\sigma$)}, is the square root of the variance.
%\begin{align}
%\sigma	&= \sqrt{ \sigma^{2} }
...
%
%\begin{align}
%E(X) 	= \mu & = \displaystyle\int_{- \infty}^{+ \infty} x \cdot f(x) \, dx	\label{expectedValueofContinuousRV}
%\end{align}
...
%\label{equivalentFormOfVariance}


\subsection{Discrete Random variables}
\label{DiscreteRandomVariablesSection}

A \term{discrete random variable} is a random variable that can only take specific (i.e. discrete) values.
Intuitively this means that we can list all of the possible values of the support of a discrete random variable.
If the support of a discrete random variables is infinite, 
we can still list consecutive elements in an ordered list.

\begin{example}{Let $X$ represent the value obtained by rolling a fair die.
The possible values that $X$ can take are $\resp{1}, \resp{2}, \resp{3}, \resp{4}, \resp{5}$, or $\resp{6}$.
$X$ can not take anything between the values that we just mentioned.
For instance, $X$ can not be \resp{2.5}. }
\end{example}

\begin{example}{Let $X$ represent the number of customers that line up at a bank everyday.
The possible values that $X$ can take are $\resp{1}, \resp{2}, \resp{3}, \ldots$.
Theoretically these values can go on to infinity, however we can 
list consecutive elements in an ordered list. 
The bank can serve $\resp{99}$ customers or $\resp{100}$ customers but not something in between these two values.}
\end{example}


\subsubsection{Probability mass function}

We can assign probabilities to the values that a random variable can take.
We do this using a \term{probability distribution function}.
For a discrete random variables the  probability distribution function is called a 
\term{probability mass function (PMF)} or mass function for short.
If $X$ is discrete random variable which takes values $x = x_{1}, x_{2}, \ldots$ the probability of $X$ taking on a specific value of $x$ is written as $P(X = x_{i})$ or $p(x_{i})$ for short.

In order for a distribution function to be a probability mass function of a discrete random variable
it must satisfy the following important properties:

\begin{termBox}{\tBoxTitle{Properties of a mass function}
Let $X$ be a discrete random variable which takes values $x = x_{1}, x_{2}, \ldots$
The probability mass function of $X$ consists of individual probabilities that have the following properties:
\begin{enumerate}
\item	$0 \leq p(x_{i}) \leq 1$ 	\label{massFuncProp1}
\item	$\displaystyle\sum_{\forall i} p(x_{i}) = 1 $		\label{massFuncProp2}
\end{enumerate}
}
\end{termBox}

Property~\ref{massFuncProp1} is telling us that the probability for each possible outcome
must be between 0 and 1.
Property~\ref{massFuncProp2} is telling us that the sum of all of the probabilities should add up to 1.
Note that we are being completely general in terms of the support of the random variable
since its support can be finite or infinite.
...
%If $X$ takes outcomes $x_1$, ..., $x_n$ with probabilities $P(X=x_1)$, ..., $P(X=x_n)$, the expected value of 
If $X$ is a discrete random variable which takes the values of outcomes $x_1$, ..., $x_n$ with probabilities $P(X=x_1)$, ..., $P(X=x_n)$, the expected value of 
$X$ is the sum of each outcome multiplied by its corresponding probability:
\begin{align}
E(X) 	= \mu & = \sum_{i=1}^{n}  x_i \cdot P(X=x_i) 	\label{expectedValueofDiscreteRV} \\
		  & = x_1 \cdot P(X=x_1) + x_2 \cdot P(X=x_2) +\ldots + x_n \cdot P(X=x_n) %\notag
\end{align}
...
It is important to make the distinction that $E(X)$ is not the same as the sample mean $\bar{x}$.
We are calculating the actual mean of the process modelled by the discrete random variable.
...
\begin{example}{Using the information in exercise~\ref{bookStoreRev}, what is the probability mass function for the revenue per student for the course}
The amount of money a single student will spend on her statistics books is a random variable, and we represent it by $X$.
The possible outcomes of $X$ are labeled with a corresponding lower case letter $x$ and subscripts. 
We will write $x_1=\$0$, $x_2=\$137$, and $x_3=\$170$, 
and these outcomes occur with probabilities $0.20$, $0.55$, and $0.25$. 
The distribution of discrete random variable $X$ (i.e. the probability mass function of $X$) 
is given in Table~\ref{statSpendDist}.
\end{example}\label{bookStorePMF}
...
\begin{example}{Using the solution to the problem in Example~\ref{bookStorePMF},
plot the probability mass function of revenue per student for the course.
}\label{bookCostDistPMF}
Using the mass function~\ref{statSpendDist}, we get point masses at each of the values 
in the support of $X$.
The height that these point masses appear at will be their corresponds probabilities.
\begin{figure}[H]
\centering
\includegraphics[width=0.69\textwidth]{02/figures2/massFunction/bookCostMassFunction.pdf}
\caption{Plot of the probability mass function for the bookstore's revenue from a single student.}
\label{bookCostDist}
\end{figure}
...
%%summarized in Figure~\ref{bookCostDist} 
%A alternative pictorial representation of the probability mass function of $X$ 
%from Example~\ref{bookStorePMF}
%is given in Figure~\ref{bookCostDist}.
%in terms of balancing weights.
%%and \ref{bookWts}.
%
%\begin{figure}[H]
%\centering
...
\begin{example}{Use the probability mass function from Example~\ref{bookStorePMF} and
the definition of expected value for a discrete random variable given in 
equation~\ref{expectedValueofDiscreteRV} to calculate the mean revenue per student for this course }
The expected value of a random variable is computed by adding each outcome weighted by its probability.
Using~\ref{expectedValueofDiscreteRV} we get:
\begin{align*}
E(X) = \mu 	&= 0 \cdot  P(X=0) + 137 \cdot  P(X=137) + 170 \cdot  P(X=170) \\
			&= 0 \cdot  0.20 + 137 \cdot  0.55 + 170 \cdot  0.25 \\
			&= 117.85
\end{align*}	
\end{example}
...
%For example, $E(X)=117.85$ represents the average amount the bookstore expects to make from a single student, which we could also write as $\mu=117.85$.

It is also possible to compute the expected value of a continuous random variable (see Section~\ref{DiscreteRandomVariablesSection}). However, it requires a little calculus and we save it for a later class.\footnote{$\mu = \int xf(x)dx$ where $f(x)$ represents a function for the density curve.}



Recall the the explanation provided in Section~\ref{sectionMeanAndVarianceOfRVs} where we looked at the mean as the center of gravity.
Figures~\ref{bookCostDist} and~\ref{bookWts} are representative of this explanation in terms
of weighted amounts balanced on a pivot.
%The idea of a center of gravity also expands to continuous probability distributions. Figure~\ref{contBalance} shows a continuous probability distribution balanced atop a wedge placed at the mean.
%%summarized in Figure~\ref{bookCostDist} 
%An alternative pictorial representation of the probability mass function of $X$ 
%from Example~\ref{bookStorePMF}
%is given in Figure~\ref{bookCostDist}.
%in terms of balancing weights.
%%and \ref{bookWts}.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.66\textwidth]{02/figures/bookCostDist/bookCostDist}
\caption{Probability distribution for the bookstore's revenue from a single student. The distribution balances on a triangle representing the average revenue per student.}
\label{bookCostDist}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.66\textwidth]{02/figures/bookWts/bookWts}
\caption{A weight system representing the probability distribution for $X$. The string holds the distribution at the mean to keep the system balanced.}
\label{bookWts}
\end{figure}
...
If $X$ is a discrete random variable which takes the values of outcomes $x_1$, ..., $x_k$ with probabilities $P(X=x_1)$, ..., $P(X=x_n)$ and expected value $\mu=E(X)$, then the variance of $X$, denoted by $V(X)$ or the symbol $\sigma^2$, is
\begin{align}
%\sigma^2 &= (x_1-\mu)^2\times P(X=x_1) + \cdots \notag \\
%	& \qquad\quad\cdots+ (x_k-\mu)^2\times P(X=x_k) \notag \\
%	&= \sum_{j=1}^{k} (x_j - \mu)^2 P(X=x_j)
...
$V(X)$\vspace{1mm}\\\footnotesize Variance\\of $X$]{\raggedright\vspace{-47mm}

$V(X)$\vspace{1mm}\\\footnotesize Variance\\of $X$}
...
\begin{exercise}
The bookstore also offers a chemistry textbook for \$159 and a book supplement for \$41. From past experience, they know about 25\% of chemistry students just buy the textbook while 60\% buy both the textbook and supplement.\footnote{(a) 100\% - 25\% - 60\% = 15\% of students do not buy any books for the class. Part~(b) is represented by the first two lines in the table below. The expectation for part~(c) is given as the total on the line $y_i\times P(Y=y_i)$. The result of part~(d) is the square-root of the variance listed on in the total on the last line: $\sigma = \sqrt{V(Y)} = \$69.28$.
\begin{center}
\begin{tabular}{rrrrr}
  \hline
...
%\subsubsection{Bernoulli random variable}\label{sectionBernouli}
%
%A Bernoulli random variable is a discrete random variable that models the outcome of a single trial in which
%there are only two possible outcomes which are either a success or a failure.
...
%We use $n$ to represent the number of trials, $x$ to represent the number of successes and
%$p$ to represent the probability of a success on any single trial.
%
%\begin{termBox}{\tBoxTitle{Binomial distribution}
%Let $X \sim \text{Bin}(n, p)$. 
%The mass function of $X$ is
%\begin{align}
%P(X = x) & = {n \choose x} p^{x} (1 - p)^{n - x}, \quad x = 0, 1, \ldots, n	\label{binomialMassFunction}
%\end{align}
%where\\
%
%\quad	$n$	is the number of trials
%
%\quad	$x$	is the number of successes we are interested in
%
%\quad	$p$	is the probability of a success on any given trial
%%\begin{tabular}{lcl}
%%	$n$	& : &	Number of trials		\\
%%	$x$	& : & Number of successes	\\
%%	$p$	& : &	The probability of a success on any given trial
%%\end{tabular}
%
%\hfill\\
%and
...
%\end{termBox}
%
%In the binomial distribution the probability of a success on any trial stays fixed at $p$.
...
%
%Note that in~\ref{binomialMassFunction}, $n$ must be a positive integer.
%If $n=1$ then we go back to the case of a Bernoulli trial.
%Also note that in~\ref{binomialMassFunction} $x$ should be a non-negative integer.
%Since $x$ is the number of successes we can have no successes in all our trials (i.e. 0 successes).
%In $n$ trials the maximum number of successes we can have is $n$ success (i.e. all trials are a success).
%One last thing to note is that when we have $n$ trials in which which we are 
%interested in the probability of $x$ successes, the remaining $(n-x)$ trials must be failures.
%
%\vfill
...
\label{ContinuousRandomVariablesSection}
%\label{DiscreteRandomVariablesSection}


A \term{continuous random variable} is a random variable that can take any
value within a specified range of values.
The support of a continuous variable has infinite precision, meaning that we can make our measurement
as fine as we would like.
Recall that in Section~\ref{DiscreteRandomVariablesSection} a discrete random variables was 
associated with a probability mass function.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{02/figures/fdicHistograms/fdicHistograms}
...
\subsubsection{From histograms to continuous distributions}
...
\subsubsection{Probabilities from continuous distributions}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{02/figures/fdicHeightContDistFilled/fdicHeightContDistFilled}
...
We associate continuous random variables with a \term{probability density function (PDF)} or density function for short.

In order for a distribution function to be a density function of a continuous random variable
it must satisfy the following important properties:


\begin{termBox}{\tBoxTitle{Properties of a distribution function}
Let $X$ be a continuous random variable. %with support $[a, b]$
We say that $f(x)$ is a probability mass function of $X$ if it satisfies all of the following properties:
\begin{enumerate}
%\item	$f(x) \geq 0,\quad \forall x \in [a, b]$				\label{distribFuncProp1}
\item	$f(x) \geq 0,\quad \forall x \in \mathbb{R}$			\label{distribFuncProp1}
\item	$P(a \leq x \leq b) = \displaystyle\int_a^b f(x) \,dx$ 	\label{distribFuncProp2}	
\item $\displaystyle\int_{-\infty}^{+\infty} f(x) \,dx = 1 $	\label{distribFuncProp3}
\item	$P( x = c) = 0, \quad \forall c \in \mathbb{R}$		\label{distribFuncProp4}	
\end{enumerate}
}
\end{termBox}


A good knowledge of integral calculus is required in order to get a proper understanding of Properties
\ref{distribFuncProp1}, \ref{distribFuncProp2}, \ref{distribFuncProp3} and \ref{distribFuncProp4}. 
Integrals and derivatives are two of the fundamental concepts of calculus and
the integral of a function gives a generalized notion of area (expressed by the
fundamental theorem of calculus).
We will explain \ref{distribFuncProp1} --- \ref{distribFuncProp4} with minimal reference
to calculus. 

Recall in section~\ref{DiscreteRandomVariablesSection} that when we plotted the mass function of a discrete random variable (such as in Example~\ref{bookCostDistPMF}) we got point masses at the values that the random variable takes.
With a continuous random variables, the density function is a continuous curve where the domain is its support.

Property~\ref{distribFuncProp1} implies that the curve of the density function must be above (or on) the 
horizontal axis.
The density curve can never go below the horizontal axis.
The definite integral of a function $f(x)$ from limits $a$ to $b$ gives the area between the curve and the 
$x$-axis (from calculus). 
Property~\ref{distribFuncProp2} implies  that for a continuous random variable, the probability of
observing values between $a$ and $b$ is given by the area under the density curve between $a$ and $b$. 
Since area under the curve gives probability, then the total area under the curve should give us the 
total probability (which is always 1).
Property~\ref{distribFuncProp3} implies that the total area under the curve should be equal to 1.
In property~\ref{distribFuncProp3} ``$-\infty$'' is notation for the lower bound of the support and ``$+\infty$'' 
is notation for the upper bound of the support.
Finally property~\ref{distribFuncProp3} implies that the probability of observing a single exact value $c$ 
is $0$.
This is because the probability of observing a single exact value is given by the area under a line
which is 0. 
...
\begin{termBox}{\tBoxTitle{Expected value of a continuous random variable}
...
The calculation of the mean in~\ref{expectedValueofContinuousRV} is the continuous analog to 
the calculation of the mean in~\ref{expectedValueofDiscreteRV} for the discrete case.
Similar to property~\ref{distribFuncProp3} in the list of properties of a distribution function
``$-\infty$'' is notation for the lower bound of the support and ``$+\infty$'' 
is notation for the upper bound of the support.

Recall the concept of the weighted balancing point discussed in Section~\ref{sectionMeanAndVarianceOfRVs}
We illustrated this concept in Figures~\ref{bookCostDist} and~\ref{bookWts} for the discrete case. 
The idea of a center of gravity also expands to continuous probability distributions. Figure~\ref{contBalance} shows a continuous probability distribution balanced atop a wedge placed at the mean.
...
\begin{termBox}{\tBoxTitle{Variance of a continuous random variable}
...
%$$ aX + bY$$
%where $a$ and $b$ are some fixed and known real numbers. 

A \term{linear combination} of two random variables is a fancy way os saying the following:

\begin{termBox}{\tBoxTitle{Linear combinations of random variables }
...
%To compute the average value of a linear combination of random variables, plug in the average of each individual random variable and compute the result:
%\begin{align*}
...
\begin{example}{Elena is selling a TV at a cash auction and also intends to buy a toaster oven in the auction. If $X$ represents the profit for selling the TV and $Y$ represents the cost of the toaster oven, write an equation that represents the net change in Elena's cash.}\label{elenaIsSellingATVAndBuyingAToasterOvenAtAnAuction}
She will make $X$ dollars on the TV but spend $Y$ dollars on the toaster oven, 
so the expression representing her net change in cash is
$$X-Y$$%\footnote{She will make $X$ dollars on the TV but spend $Y$ dollars on the toaster oven: $X-Y$.}
\end{example}
...
\subsection{Expected value of linear combinations of random variables}
...
\begin{termBox}{\tBoxTitle{Expected Value of a linear combination of random variables}
Let $X$ and $Y$ be random variables and let $a$ and $b$ be any two real numbers. 
To compute the average value of a linear combination of $X$ and $Y$, plug in the average of each individual random variable and compute the result:
\begin{align*}
 E(aX + b Y) & = a \cdot E(X) + b \cdot E(Y)
\end{align*}
%Recall that the expected value is the same as the mean, e.g. $E(X) = \mu_X$.}
}
\end{termBox}
...
\begin{example}{It takes John an average of 18 minutes each day to commute to work. What would you expect his average commute time to be for the week?}\label{expectationJohnsCommute}
...
\begin{exercise}\label{expectationElena}
Recall Example~\ref{elenaIsSellingATVAndBuyingAToasterOvenAtAnAuction}.
...
With reference Example~\ref{expectationJohnsCommute} and the solution to Exercise~\ref{expectationElena},
...
The variance of a linear combination of $X$ and $Y$ is given by
\begin{align}
V(aX + bY) = a^2  V(X) + b^2  V(Y)		\label{varianceLinearCombination}
\end{align}
...
%This is because $\sqrt{a^{2}} = \pm a$
%and
%$\sqrt{b^{2}} = \pm b$ however 
%\begin{align*}
%V(aX + bY) = a^2\times V(X) + b^2\times V(Y)
%\end{align*}
We reiterate the importance of assuming that  the random variables are independent in~\ref{varianceLinearCombination}. %this equality 
If independence doesn't hold, then more advanced methods are necessary. Equation~\ref{varianceLinearCombination} can be used to compute the variance of Leonard's monthly return:
\begin{align*}
V(6000 X + 2000 Y)
	&= 6000^2\times V(X) + 2000^2\times V(Y) \\
	&= 36,000,000\times 0.0072 + 4,000,000\times 0.0027 \\
	&= 270,000
\end{align*}

The standard deviation is computed as the square root of the variance.
%In~\ref{varianceLinearCombination} notice that $a$ and $b$ are squared.
%Squaring any real number results in a non-negative value.
%Therefore we should be careful when we write a general expression for 
%the standard deviation since $a^{2}$ and $b^{2}$ are non-negative regardless of whether $a$ and $b$ 
%are non-negative or not.
%Since standard deviation is a non-negative quantity 
%we should only consider the positive root of 
%$V(aX+bY)$.
%This is given by 
In the case of Leonard's monthly return
\begin{align*}
SD(6000 X + 2000 Y)
	&= \sqrt{V(6000 X + 2000 Y)}\\
	&= \sqrt{270,000} = \$520
\end{align*}
